         ...
         std::shared_ptr<core_interfaces::srv::UiMotor::Response> response)
{
  ...
  semaphore.acquire();
  RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "Leg2Enterance");
  ...
  universialControl(13, 4, request->leg1, 50, 0, 0, serial);
...
int main(int argc, char **argv)
{
  ...
  // Create a ROS 2 node
  std::shared_ptr<rclcpp::Node> node = rclcpp::Node::make_shared("leg2");

  // Create a service for controlling motors
  rclcpp::Service<core_interfaces::srv::UiMotor>::SharedPtr service = 
  node->create_service<core_interfaces::srv::UiMotor>("motorcontrol2", &legcontrol);

  // Log a message indicating that the server is ready to control Leg 1
  RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "Ready to control Leg 2");
